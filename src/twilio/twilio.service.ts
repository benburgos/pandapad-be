...
export default class TwilioService {
  ...
  constructor(private configService: ConfigService) {
    this.client = new Twilio(
      this.configService.get(process.env.SERVICE_TWILIO_KEY_SID),
      this.configService.get(process.env.SERVICE_TWILIO_KEY_SECRET),
      {
    ...
    this.conversationsServiceId = this.configService.get(
      process.env.SERVICE_TWILIO_CONVERSATION_SID,
    );
  }
